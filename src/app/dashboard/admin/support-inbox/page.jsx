...
export default function AdminSupport() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.thread) {
          ...
          setActiveThread(updatedThread);
          setThreads((prev) =>
            prev.map((t) => (t.userId === thread.userId ? updatedThread : t))
          );
        } else {
  ...
  const getLastMessage = (thread) => {
    return (
      thread.lastMessage ||
      (thread.messages && thread.messages[thread.messages.length - 1]?.text) ||
      "No messages yet"
    );
  };

  return (
    <div className="flex h-[calc(100vh-100px)] bg-background text-foreground overflow-hidden transition-colors">
      {/* Sidebar */}
      <Card className="w-96 m-4 flex flex-col border border-border bg-card text-foreground overflow-hidden">
      ...
      {/* Chat Area */}
      <Card className="flex-1 m-4 flex flex-col border border-border bg-card text-foreground overflow-hidden">
        {!activeThread ? (
                ...
                <div className="space-y-4">
                  ...
                  {userTyping && (
                    <div className="flex justify-start">
                      <div className="max-w-[70%] rounded-lg px-4 py-2 bg-muted border border-border">
