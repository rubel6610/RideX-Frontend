...
export default function PassengerDash() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [ridersRes, paymentsRes, reviewsRes, ridesRes] = await Promise.all([
          fetch("http://localhost:5000/api/riders"),
          fetch("http://localhost:5000/api/payment/all"),
          fetch("http://localhost:5000/api/ride-reviews"),
          fetch("http://localhost:5000/api/rides"), // Active bookings source
        ]);
  ...
  if (loading) return <p className="text-center text-lg">⏳ ডাটা লোড হচ্ছে...</p>;

  // ---------- Stats হিসাব ----------
  const totalRiders = riders.length;

  // শুধুমাত্র "accepted" স্ট্যাটাস যেগুলোর
  const activeBookings = rides.filter(r => r.status === "accepted").length;

  // Total Spent: nested rideDetails.fareBreakdown.totalAmount
  ...
  const monthlySpending = payments.map(p => ({
    ...
    amount: p.amount ?? p.totalAmount ?? p.subtotal ?? p.rideDetails?.fareBreakdown?.totalAmount ?? 0
  }));
  ...
  const stats = [
    { title: "Total Rides", icon: MapPin, value: totalRiders },
