...
export default function RiderSupportChat() {
  const { user } = useAuth();
  ...
  const fetchThread = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_SERVER_BASE_URL}/support/thread/${user.id}?userType=rider`
      );
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_BASE_URL}/support/send`, {
        ...
        body: JSON.stringify({
          userId: user.id,
          text: messageText,
          userType: 'rider', // Specify rider type
        }),
      });
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto mt-8 border-border bg-card">
      <CardHeader className="border-b border-border bg-gradient-to-r from-primary to-accent">
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center gap-2 text-white">
            <MessageCircle className="h-6 w-6" />
            Rider Support Chat
          </CardTitle>
          <Badge variant={status === 'waiting' ? 'destructive' : 'secondary'} className="bg-white/20 text-white border-white/30">
            {status}
          </Badge>
        </div>
      </CardHeader>
          ...
          <div className="space-y-4">
                  ...
                  >
                    ...
                    <div
                      className={`text-xs mt-1 ${
                        msg.sender === 'rider' ? 'text-white/80' : 'text-muted-foreground'
                      }`}
                    >
            ...
            {adminTyping && (
              <div className="flex justify-start">
                <div className="max-w-[80%] rounded-lg px-4 py-2 bg-card text-foreground border border-border">
